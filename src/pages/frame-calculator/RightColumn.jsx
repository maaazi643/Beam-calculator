...
export default function RightColumn() {
  ...
  const toggleShowConfigHandler = () => {
    dispatch(
      frameActions.set({
        key: "showRightSpanConfig",
        value: !showRightSpanConfig,
      })
    );
  };
  ...
  const applySpanHandler = () => {
    ...
    const newFrameProperties = { ...frameProperties, rightColumn: rightColumn };
    dispatch(
      frameActions.set([
      ...
      >
        ...
        <UpArrow
          className={`transform transition-transform ${
            !showRightSpanConfig ? "rotate-180" : ""
          }`}
        />
      </button>
      <motion.div
        initial="hidden"
        animate={showRightSpanConfig ? "visible" : "hidden"}
...
function LeftItem() {
  ...
  const lengthChangeHandler = (value) => {
    dispatch(
      frameActions?.set({
        key: "rightColumn",
        value: { ...rightColumn, length: value },
      })
    );
  };
  ...
  const flexuralRigidityChangeHandler = (value) => {
    dispatch(
      frameActions?.set({
        key: "rightColumn",
        value: { ...rightColumn, flexuralRigidity: value },
      })
    );
  };
  ...
  const changeSupportType = (value) => {
    ...
    dispatch(
      frameActions?.set({
        key: "rightColumn",
        value: { ...rightColumn, support: newSupport },
      })
    );
  };
  ...
  const changeLoadType = (value) => {
    ...
    dispatch(
      frameActions?.set({
        key: "rightColumn",
        value: { ...rightColumn, loading: newLoading },
      })
    );
  };
  ...
  const changeSinking = (value) => {
    ...
    dispatch(
      frameActions?.set({
        key: "rightColumn",
        value: { ...rightColumn, support: newSinking },
      })
    );
  };
...
function UniformVaryingLoadSettings() {
  ...
  const changeDistanceFromTop = (distanceFromTop) => {
    const newLoad = { ...loading, distanceFromTop: distanceFromTop };
    dispatch(frameActions.set({ key: "rightColumn", value: newLoad }));
  };
